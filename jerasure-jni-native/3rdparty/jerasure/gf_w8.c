...
gf_w8_log_multiply(gf_t *gf, uint32_t a, uint32_t b) {
    ...
    return (a == 0 || b == 0) ? 0 : ltd->antilog_tbl[(unsigned) (ltd->log_tbl[a] +
                                                                 ltd->log_tbl[b])];
}
...
static
void
gf_w8_double_table_multiply_region(gf_t *gf, void *src, void *dest, gf_val_32_t val, int bytes,
                                   int xor) {
    uint16_t *base;
    uint32_t b, c, vc, vb;
    gf_internal_t *h;
...
static
void
gf_w8_composite_multiply_region_alt(gf_t *gf, void *src, void *dest, gf_val_32_t val, int bytes,
                                    int xor) {
    ...
    base_gf->multiply_region.w32(base_gf, (uint8_t *) rd.s_start + sub_reg_size, rd.d_start, val1,
                                 sub_reg_size, 1);
    base_gf->multiply_region.w32(base_gf, rd.s_start, (uint8_t *) rd.d_start + sub_reg_size, val1,
                                 sub_reg_size, xor);
...
gf_w8_composite_inverse(gf_t *gf, gf_val_32_t a) {
    ...
    if (a0 == 0) {
        ...
        tmp = (base_gf->multiply.w32(base_gf, a1, a0inv) ^
               base_gf->multiply.w32(base_gf, a0, a1inv) ^ h->prim_poly) & 0xf;
...
static
void
gf_w8_composite_multiply_region(gf_t *gf, void *src, void *dest, gf_val_32_t val, int bytes,
                                int xor) {
    gf_region_data rd;
    gf_internal_t *h = (gf_internal_t *) gf->scratch;
    gf_t *base_gf = h->base_gf;
...
static
void
gf_w8_bytwo_p_nosse_multiply_region(gf_t *gf, void *src, void *dest, gf_val_32_t val, int bytes,
                                    int xor) {
...
static
void
gf_w8_bytwo_b_nosse_multiply_region(gf_t *gf, void *src, void *dest, gf_val_32_t val, int bytes,
                                    int xor) {
